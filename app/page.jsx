...
export default function POSSystem() {
  ...
  // --- Views (Mobile Optimized - Visual Balanced) ---
  const POSView = () => (
    <div className="flex h-full flex-col md:flex-row overflow-hidden">
      <div className="flex-1 flex flex-col min-w-0 bg-gray-50/50">
        <header className="h-14 md:h-16 bg-white border-b pl-4 pr-6 flex items-center justify-between shrink-0 z-10">
        ...
        <div className="px-3 py-2 bg-white border-b flex gap-2 overflow-x-auto scrollbar-hide shrink-0 pr-24">
          {CATEGORIES.map(cat => (
        ...
        {/* Product Grid (Visual Balanced for Mobile) */}
        <div className="p-3 md:p-6 flex-1 overflow-y-auto pb-40 md:pb-6">
            ...
            {filteredProducts.map(p => (
              <button key={p.id} onClick={() => addToCart(p)} disabled={p.stock <= 0} className={`relative bg-white p-2 md:p-3 rounded-xl shadow-sm border border-gray-100 flex flex-row md:flex-col items-center gap-3 md:gap-0 md:justify-between h-auto md:h-48 transition-all active:scale-95 ${p.stock <= 0 ? 'opacity-60 grayscale' : ''}`}>
                ...
                <div className="text-left md:text-center w-full flex-1">
                  ...
                  <p className={`text-${PRIMARY_COLOR}-600 font-extrabold text-base md:text-lg`}>฿{p.price}</p>
                </div>
